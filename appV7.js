...
init: function() {
  ...
  this.subTodoTemplate = Handlebars.compile(document.querySelector("#sub-todo-template").innerHTML);
  this.events();
  this.render();
},
...
newInputEvent: function() {
  ...
  var newSubInput = document.getElementById('new-input');
  var newInput = document.getElementById('first-input');
  var subTreeInput = document.getElementById('subTree-input');
  if (newInput) {
    newInput.addEventListener('keyup', callCreate);
    newInput.focus();
  }
  ...
  if (subTreeInput) {
    subTreeInput.addEventListener('keyup', callCreate);
...
whatToRender: function() {
  ...
  if (this.todos.length === 0) {
    ...
    this.todos.forEach(function(todo) {
      if (todo.subTree === true) {
        document.getElementById(todo.id).innerHTML = (App.subTodoTemplate(todo.subArray));
      }
    })
  }
  this.newInputEvent();
...
findParentTodo: function(el) {
  var preId = el.closest('ul').closest('li');
  var id = preId.getAttribute('data-id');
  var todos = this.todos;
  var i = todos.length;
...
create: function (e) {
  //determine if from main branch or from subtree
  var input = e.target;
  var val = input.value;
  ...
  if (ul.getAttribute('id') === 'main-tree') {
    this.todos.push({
      id: util.uuid(),
      title: val,
      completed: false,
    ...
    var todo = this.todos[App.findParentTodo(e.target)];
    todo.subArray.push({
      id: util.uuid(),
      title: val,
      completed: false,
